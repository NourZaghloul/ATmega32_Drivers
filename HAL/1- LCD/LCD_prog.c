...
void CLCD_voidSendCmd(uint8 Copy_u8Cmd)
{
	...
	DIO_u8SetPinValue(CLCD_u8CTRL_PORT, CLDC_u8RW_PIN, DIO_u8PIN_LOW);
#endif
...
void CLCD_VoidSendNumber(sint32 Copy_s32Number)
{
	...
	{
		Local_u8RightDigit = (uint32)Copy_s32Number % 10;  				/*get the right most digit*/
...
uint8 CLCD_u8SendSpecialCharacter(uint8 Copy_u8LocationNum, uint8* Copy_pu8Pattern, uint8 Copy_u8XPos, uint8 Copy_u8YPos)
{
	...
	if(Copy_pu8Pattern != NULL)
	{
		...
		uint8 Local_u8CG_RAM_Address = Copy_u8LocationNum * 8;

		/*Set bit 6 for CGRAM address command*/
		SET_BIT(Local_u8CG_RAM_Address, 6u);

		/*Set ACGRAM Address*/
		CLCD_voidSendCmd(Local_u8CG_RAM_Address);
...
{
	DIO_u8SetPortValue(CLCD_u8CTRL_PORT, DIO_u8PORT_LOW);
	DIO_u8SetPortValue(CLCD_u8DATA_PORT, DIO_u8PORT_LOW);
}
